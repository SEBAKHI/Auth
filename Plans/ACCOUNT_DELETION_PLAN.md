...
10. **Operational hardening:** hosting backup retention ≤ 6 months; extend the DataProtection certificate/key-ring backup runbook to name the new dependents (DEKs, HMAC key); add the post-restore re-apply verification step to the restore runbook; record `PolicyVersion 2026.07` and the §5 retention table in the privacy-policy disclosure page linkage (R6). **Load-bearing for R6 since the §9 entry-point decision:** the public wizard is linked from no in-product screen, so its discoverability rests entirely on (a) a `/delete-account` link in the published privacy policy and (b) the app-store listing's data-deletion URL field (Google Play's policy requires a URL usable without installing the app or signing in). Until both are set, R6's "standalone public web page" is satisfied only by direct-URL access.
