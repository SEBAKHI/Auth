...
| Profile page (`packages/account/src/pages/profile/profile-page.tsx`) | `Tabs` (Account / Security / Sessions), `Card` sections, `react-hook-form` + `zod` + `Form`/`FieldGroup`, `sonner` toasts, `Skeleton`, `@authsystem/api` client |
...
| Shared (`packages/api`) | Env + contract | — | `APPLE_SERVICES_ID` added to `@authsystem/api/env` beside `GOOGLE_CLIENT_ID`; generated API types refreshed for the new endpoints/fields |
