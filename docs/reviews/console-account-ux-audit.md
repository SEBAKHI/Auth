...
5. **تدقيق WCAG الآلي — منفَّذ؛ واليدوي ما زال مفتوحًا.** `@axe-core/playwright` يفحص ثلاث شاشات × سمتين × اتجاهين مقابل `wcag2a/aa` و`wcag21a/aa` و`wcag22aa` — اثنتا عشرة حالة. أصلح الفحص عيبين: مقابض تغيير عرض الأعمدة كانت `role="separator"` قابلة للتركيز بلا `aria-valuenow` (المطلوب لدورها)، ومحدِّد عدد الصفوف كان `combobox` بلا اسم مقروء إطلاقًا. **ما لا يبلغه الفحص الآلي:** ترتيب التنقّل بلوحة المفاتيح، وأين يستقرّ التركيز بعد إغلاق حوار، ووضوح رسالة الخطأ حين تُقرأ صوتيًا، ومعنًى يحمله اللون وحده — وكلها تبقى فحصًا يدويًا لم يُجرَ.

   **وفجوتان في لوحة الألوان لا أملك إصلاحهما:** `--muted-foreground` على `--muted` يعطي `4.34:1` والمطلوب `4.5`، و`--destructive` على تدرّجه بنسبة `10%` يعطي `4.0`. كلاهما مُعرَّف في `preset.css` ومستعمَل في اثني عشر مكوّنًا، فالتصحيح تغميقُ رمزٍ واحد — قرار مالك نظام التصميم، ويمنعه صراحةً قيد «لا ألوان مخصّصة» في هذا المستودع. استُثنيا **بزوج اللون لا بالمحدِّد**، حتى لا تتّسع قائمة الاستثناءات كلما ظهر الزوج في مكان جديد، ولا تبتلع عطلًا مختلفًا على العنصر نفسه؛ وثبت أن أيّ فشل تباين آخر ما زال يُسقط الحزمة.
...
- **شاشة الدخول كانت تحمّل الواجهة المصادَقة كاملة — عولج.** `routes.tsx` في التطبيقين كان يستورد الغلاف استيرادًا ثابتًا، فيدخل في الحزمة الأولى مع البحث الشامل وcmdk وشريط التنقّل وقوائم Radix. صار `lazy` مثل كل صفحة تحته. المقيس: Console من `926,137` إلى `789,175` بايت (**‎−136,962**، ‎−14.8٪)، وAccounts من `855,005` إلى `765,249` (**‎−89,756**، ‎−10.5٪). حارس `login-payload.spec.ts` يمنع عودته، وميزانيته شُدَّت إلى `850,000`. ما تبقّى إجباريًا على المسار: React وnext locale وreact-query وi18n، إضافةً إلى `TooltipProvider` و`Toaster` اللذين يركّبهما `App.tsx` للتطبيق كله.

  **وتصحيح مهم لهذا البند:** الكسل لا يشتري ما يبدو أنه يشتريه. الموجّه يحلّ `lazy` لكل مسار مطابَق **قبل** أن يرسم شيئًا، و`RequireAuth` مكوّن يعمل وقت الرسم — فمن يكتب أصل الموقع مجرّدًا يحمّل الغلاف ولوحة التحكم وrecharts، **ثم** يُحوَّل إلى `/login`. المقيس لزائر غير مسجَّل:

  | ‏نقطة الدخول | ‏قبل | ‏بعد |
  |---|---|---|
  | ‏`/login` | ‏`926,137` | ‏`789,175` |
  | ‏`/` | ‏`1,366,439` | ‏`1,371,835` |
  | ‏`/users` | ‏`1,141,841` | ‏`1,147,807` |

  أي أن المكسب يخصّ من يصل إلى `/login` مباشرة، وأن المسارين الآخرين ازدادا `5.4 kB` تقريبًا (حدّ حزمة إضافي). وكانت النسخة الأولى من الحارس تقيس `/login` وحده — أي المسار النظيف أصلًا — فأضيفت نقطتا الدخول الأخريان بميزانيتين تسجّلان الواقع لا المرجوّ، وتنخفضان ولا ترتفعان. **الإغلاق الفعلي** يتطلّب موجّهًا للزوّار غير المسجَّلين يُركَّب حتى تُعرف الجلسة، وهو يمسّ عقد `returnTo` — قرار، لا ترتيب.

  **ثم عولج الجزء الأكبر بلا مساس بالتوجيه:** مكتبة الرسوم (`~325 kB`) كانت تدخل مع لوحة التحكم لأن الصفحة تستورد تبويباتها الخمسة استيرادًا ثابتًا. صارت التبويبات كسولة داخل الصفحة خلف `Suspense`، فخرجت المكتبة من مسار الزائر غير المسجَّل. المقيس بعدها:

  | ‏نقطة الدخول | ‏الأصل | ‏بعد كسل الغلاف | ‏بعد كسل التبويبات |
  |---|---|---|---|
  | ‏`/login` | ‏`926,137` | ‏`789,175` | ‏`790,301` |
  | ‏`/` | ‏`1,366,439` | ‏`1,371,835` | ‏**`959,442`** |
  | ‏`/users` | ‏`1,141,841` | ‏`1,147,807` | ‏`1,149,821` |

  أي أن `/` انخفض `406,997` بايتًا (‎−29.7٪) عن الأصل. ما تبقّى على ذلك المسار هو الغلاف وصفحة لوحة التحكم نفسها؛ إخراجهما يحتاج موجّه الزوّار غير المسجَّلين، وهو ما زال قرارًا مفتوحًا.
