...
public interface IApiKeyRepository
{
    ...
    /// </summary>
    Task<IReadOnlyList<ApiKey>> GetByApplicationAsync(Guid applicationId, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    Task AddScopeAsync(ApiKeyScope scope, CancellationToken cancellationToken = default);

    /// <summary>
    /// Removes a scope from an API key.
    /// </summary>
    Task RemoveScopeAsync(Guid apiKeyId, Guid permissionId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets the permission codes for an API key.
    /// </summary>
    Task<IReadOnlyList<string>> GetScopesAsync(Guid apiKeyId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Records the last usage of an API key.
    /// </summary>
    Task RecordUsageAsync(Guid apiKeyId, CancellationToken cancellationToken = default);
}
