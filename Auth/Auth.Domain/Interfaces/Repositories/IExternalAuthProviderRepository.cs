...
public interface IExternalAuthProviderRepository
{
    ...
    /// </summary>
    Task<IReadOnlyList<ExternalAuthProvider>> GetAllEnabledAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets an external authentication provider by its code.
    /// </summary>
    Task<ExternalAuthProvider?> GetByCodeAsync(string code, CancellationToken cancellationToken = default);
}
