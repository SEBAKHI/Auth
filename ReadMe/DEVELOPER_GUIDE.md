...
| `MaxFailedAttempts` | Wrong passwords before a lock — counted per account for the lock itself, and separately per client address, so an address that keeps failing is refused even while the account is open |
...
- **Lockout shuts out strangers, not the owner.** Five wrong passwords (`Password:MaxFailedAttempts`) lock the account for 15 minutes (`Password:LockoutDurationMinutes`), and it unlocks itself on the first attempt after the window. Since September 2026 the *automatic* lock — the one the failure counter raises — tells a familiar source apart from a stranger: a client address the account signed in from within the last 30 days, or a device that holds a **live** session for it, may still sign in while the lock stands, because the lock exists to stop the strangers whose failures raised it. When such a sign-in succeeds (password verified, or provider token accepted) the lock is cleared in full; the strangers' addresses stay refused by the per-address ceiling. An **administrator's lock** (`POST /api/v1/users/{id}/lock` — no expiry, counter untouched) is never relaxed this way, and a completed password reset clears only the automatic lock. Separately, the same five wrong passwords are counted **per client address** regardless of the lock: an address that reaches the count is refused — before the next password verification — until one window has passed since its last wrong guess; refusals themselves do not extend the window.
...
**One assumption deserves stating bluntly: the API trusts the `X-Forwarded-For` header from whoever sends it.** It takes the first entry of that header as the client address, and the forwarded-headers middleware is configured with no list of known proxies or networks. So any caller who can reach the API directly can choose which rate-limit bucket they land in, and can write any address they like into the audit log. Since September 2026 the lockout rules key on that same address too: a direct caller could present a lock-out victim's usual address to pass an automatic lock, or spend another address's allowance of wrong passwords. **This is safe only when the API is unreachable except through the gateway**, which overwrites the header (`AddXForwardedFor(Set)`) and refuses direct callers without the gateway token. If the API is directly reachable on your network, treat every per-address protection here as advisory.
