...
export const fa: TranslationResources = {
  ...
  notifications: {
    ...
    manageVariablesHint:
      "این متغیرهایی است که این نوع اعلان به قالب‌هایش می‌دهد. کاتالوگ قرارداد با کد ارسال است: قالب فقط می‌تواند به متغیرهای فهرست‌شده در اینجا ارجاع دهد.",
    variablesContextHint:
      "همیشه در دسترس (نیازی به افزودن نیست): {{ Platform.Name }}، {{ Application.Name }}، {{ Application.Code }}، {{ SenderName }}، {{ Year }}. مقادیر مخصوص گیرنده و جریان توسط کد ارسال تأمین می‌شود.",
    customVariableWarning:
      "متغیری که اینجا اضافه می‌کنید تنها زمانی داده دارد که کد ارسال (یا یک شیء زمینه بالا) آن را تأمین کند؛ در غیر این صورت خالی نمایش داده می‌شود.",
    variableName: "نام",
